...
`/listener gui` 会先打开模式选择：新手向导通过事件/动作菜单和预览页面创建规则，高级编辑则用于查看已有规则、右键启停、测试和精细调整。向导只在规则 ID 和自定义内容时使用聊天输入，保存会自动写回 `plugins/Listener/config.yml` 并重载规则。编辑器和菜单点击会再次检查 `listener.admin`，普通玩家无法使用。
